...
const SideNav: React.FC<SideNavProps> = ({ 
  ...
  return (
    <>
      {/* Overlay background for closing SideNav, rendered only when overlay is enabled */}
      {overlay && (
        <Animated.View 
          style={[overlayAnimatedStyle, styles.overlay]} 
        >
          <TouchableOpacity onPress={toggle} style={styles.touchableOverlay} />
        </Animated.View>
      )}
      
      {/* SideNav container with configurable width and position */}
      <PanGestureHandler onHandlerStateChange={useCallback((event: PanGestureHandlerGestureEvent) => {
